...
function insertPackages(){   
        ...
        if ($name){
            ...
            /* Insert the record  */
            $query="INSERT INTO jeepsafari (name,description,image) Values(?,?,?)";
...
function updatePackages(){
    global $con;
        ...
        if(!isset($_FILES['files'])){
            $query = "Select * from jeepsafari where id = '$id'";
            $rs = mysqli_query($con,$query);
            $row = mysqli_fetch_assoc($rs);
            $pro_images = $row['image'];
            $uploadedFiles[0] = $pro_images;
            $query="Update jeepsafari set name=?,description=?,image=? where id=?";
            $stmt = $con->prepare($query);
...
function deleteRecords($id){
	...
	$query =  "DELETE FROM jeepsafari WHERE id=$id";
